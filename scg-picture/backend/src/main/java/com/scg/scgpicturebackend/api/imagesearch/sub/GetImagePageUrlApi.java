package com.scg.scgpicturebackend.api.imagesearch.sub;
...
//获取以图搜图地址 step1
/*这个后续考虑使用开放的api或者付费api 这个目前有爬虫限制好像 有概率失败*/
@Slf4j
public class GetImagePageUrlApi {
    ...
    public static String getImagePageUrl(String imageUrl) {
        ...
        try {
            // 2. 发送请求
            Map<String, Object> result = getResponse(url, formData);
            boolean isFaild = true;
                //多重试几次 因为有可能会失败
                for(int i = 0;i<9;i++){
                    // 3. 处理响应结果
                    if (result == null || !Integer.valueOf(0).equals(result.get("status"))) {
                        log.error("第{}次调用接口失败", i);
                        result = getResponse(url, formData);
                    }else{
                        log.info("第{}次调用接口成功，接口{}", i,result.get("data"));
                        isFaild = false;
                        break;
                    }
                }
    ...
    private static Map<String, Object> getResponse(String url, Map<String, Object> formData) {
